...
import com.sba.user.exception.DataValidationException;
import com.sba.user.exception.ResourceExistException;
import com.sba.user.exception.ResourceNotFoundException;
import com.sba.user.exception.ServiceUnavailableException;
import com.sba.user.model.Constants;
...
public class UserService implements UserDetailsService{
	...
	public User signup(User user, HttpServletRequest request) {
		...
		if (!(user.getRole().toUpperCase().equals(Constants.UserRole.ROLE_ADMIN.getRole())
				|| user.getRole().toUpperCase().equals(Constants.UserRole.ROLE_USER.getRole())
				|| user.getRole().toUpperCase().equals(Constants.UserRole.ROLE_MENTOR.getRole()))) {
		...
		} else if (user.getRole().toUpperCase().equals(Constants.UserRole.ROLE_MENTOR.getRole())) {
		...
		if (userRepository.findByName(user.getUserName(),user.getRole()) != null)
	...
	public User updateProfile(User user) {
		...
		return userRepository.findById(user.getId()).map(oldUser -> {
			...
			if (!user.getActive().equals(oldUser.getActive()))
				...
				if (oldUser.getRole().toUpperCase().equals(Constants.UserRole.ROLE_MENTOR.getRole())) {
