...
import com.sba.training.client.PaymentServiceFeignClient;
import com.sba.training.client.SearchServiceFeignClient;
import com.sba.training.client.SkillServiceFeignClient;
import com.sba.training.client.UserServiceFeignClient;
import com.sba.training.entity.Training;
import com.sba.training.exception.DataValidationException;
import com.sba.training.exception.ResourceExistException;
import com.sba.training.exception.ResourceNotFoundException;
import com.sba.training.exception.ServiceUnavailableException;
import com.sba.training.model.*;
import com.sba.training.repo.TrainingsRepository;
...
public class TrainingService {
	...
	@Autowired
	private SearchServiceFeignClient searchProxy;

	@Autowired
	private SkillServiceFeignClient skillProxy;

	@Autowired
	private PaymentServiceFeignClient paymentProxy;
	...
	public Training updateTraining(Training training, String authToken) {
		...
		if (oldTraining.getStatus().equals(Constants.TrainingStatus.PROPOSED.getStatus())
				&& (training.getStatus().equals(Constants.TrainingStatus.CONFIRMED.getStatus())
						|| training.getStatus().equals(Constants.TrainingStatus.REJECTED.getStatus()))) {
			mailFlag = true;

			if (training.getStatus().equals(Constants.TrainingStatus.CONFIRMED.getStatus())
					|| training.getStatus().equals(Constants.TrainingStatus.REJECTED.getStatus())) {
		...
		} else if ((training.getStatus().equals(Constants.TrainingStatus.TRAINING_STARTED.getStatus())
				|| training.getStatus().equals(Constants.TrainingStatus.COMPLETED.getStatus()))
			...
			if(!oldTraining.getStatus().equals(Constants.TrainingStatus.TRAINING_STARTED.getStatus()))
				throw new DataValidationException(Translator.toLocale("error.validate.statusOldstatus"));
			if (training.getStatus().equals(Constants.TrainingStatus.TRAINING_STARTED.getStatus())) {
				oldTraining.setStatus(training.getStatus());
				updateFlag = true;
			}else {
			...
			if (!oldTraining.getStatus().equals(Constants.TrainingStatus.CONFIRMED.getStatus()))
				throw new DataValidationException(
						Translator.toLocale("error.validate.trainingStatus", Constants.TrainingStatus.TRAINING_STARTED.getStatus()));
	...
	public void updateTrainingToNotCompleted(Training trainingObj) {
		...
		trainingObj.setStatus(Constants.TrainingStatus.NOT_COMPLETED.getStatus());
	...
	public void updateTrainingToStarted(Training trainingObj) {
		...
		trainingObj.setStatus(Constants.TrainingStatus.TRAINING_STARTED.getStatus());
		trainingRepository.save(trainingObj);
	...
	public String updateTrainingProgress(Training trainingObj) {
		...
		if (trainingObj.getProgress().equals(100))
			trainingObj.setStatus(Constants.TrainingStatus.COMPLETED.getStatus());
	...
	public TrainingDtls fallback_findByMentorIdAndStatus(Long mentorId, List<String> trainingStatus,
